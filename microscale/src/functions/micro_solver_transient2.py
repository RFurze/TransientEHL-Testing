...
class MicroTransientSolver_nondim:
    ...
    def _solve_micro_step(
        ...
        *,
        ...
        # Assemble once-per-Newton (closure so we can rebuild with different α and β_s)
        def newton_solve(alpha_val: float, beta_s: float):
            u = self.u
            v = self.v
            lam = self.lam
            mu = self.mu
            ...
            Ux = self.Ux_nd
            Uy = self.Uy_nd
            # Effective cavitation threshold used in the residual ONLY.
            # Non-dimensionalisation remains based on the TARGET beta (self.beta_nd).
            beta_eff_nd = self.beta_nd * beta_s
        ...
        if continuation:
            # --- β-continuation with FIXED α=1e-2 ---
            alpha_final = 1.0e-2
            START_SCALE = 10000.0
            REDUCTION   = 10.0     # 1000 -> 100 -> 10 -> 1
            beta_s = START_SCALE
            steps = 0
            while beta_s > 1.0 + 1e-12 and steps < 30:
                newton_solve(alpha_final, beta_s)
                beta_s = max(1.0, beta_s / REDUCTION)
                steps += 1
            # final solve at target β (beta_s == 1.0)
            newton_solve(alpha_final, 1.0)
        else:
            # Transient or external call: fixed α (default 1e-2), target β only
            alpha_final = fixed_alpha if fixed_alpha is not None else 1.0e-2
            newton_solve(alpha_final, 1.0)
    ...
    def run_steady(self) -> None:
        """
        Solve a single, steady Reynolds problem using:
        • FIXED α = 1e-2
        • β-continuation: 1000×β → β (target), nondim remains target-based
        """
        ...
        (_, _, _, h_steady, _, _, p_steady, _, _, _, alpha_final) = (
            self._solve_micro_step(
                ...
                continuation=True,              # ← β-continuation ON
                fixed_alpha=1.0e-2,             # ← α fixed at 1e-2
                p_guess=None,
                beta_scale=1000.0,              # initial scale (used by loop anyway)
            )
        )
