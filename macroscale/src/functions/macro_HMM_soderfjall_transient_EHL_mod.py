...
parameters["form_compiler"]["quadrature_degree"] = 8 #4 seems to be reasonable?
import numpy as np
        ...
        if not full:
            ...
            if not getattr(self, "_beta_freeze_active", False):
                ...
                if (getattr(self, "_relax_k_counter", 0) % k) == 0:
                    ...
                    print(f"[BETA] New beta {proposed/self.material_properties.p0:.6g}, old {float(self.beta):.6g}, alpha = {self.alpha}")
