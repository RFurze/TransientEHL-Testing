...
class meshfn:
    ...
    def initialise_mesh(self):
        ...
        # Compute normalized Rout = outer_radius / inner_radius in your original (pre-scale) units.
        # If your XDMF was built with inner radius = 1.0 and outer radius = 1.2 (non-dimensional),
        # then simply:
        Rout_nd = 1.2
        ...
        zcut  = ZCut(self.mesh_parameters, Rout_nd=Rout_nd, tol_cut=20*self.mesh_parameters.tol)
        # Mark in an overwrite-safe order. If you want cut-plane to supersede others, mark it last.
        inner.mark(marker, 1)
        outer.mark(marker, 2)
        zcut.mark(marker,  2)   # fold z=0 plane into outer
        cut.mark(marker,   3)

        # If you want a submesh of the **inner** only:
        ...
        # submesh3 = MeshView.create(marker, 3)
        # File("meshC.pvd") << self.meshC
        # File("meshF.pvd") << self.meshF
        # File("mesh2.pvd") << submesh2
        # File("mesh3.pvd") << submesh3
        # File("facet_markers.pvd") << marker
...
def roelands(p):
    ...
    p_r = Constant(0.5e8)  # Pa    (Roelands reference pressure; POSITIVE)
...
class EHLSolver:
    def __init__(self, meshin, material_properties, solver_params, output_folder):
        ...
        self.h_regularisation_param = 3e-3  # baseline S before any decay
        # --- regularisation decay controls/state ---
        self.reg_decay_trigger_factor = 50.0   # start decay when p-res <= factor * inner_tol_p
        self.reg_decay_iters = 12             # how many inner iterations to decay over
        ...
        self.inner_tol_p = 1e-4       # pressure mixing residual target
        self.inner_tol_d = 1e-4       # deformation mixing residual target
        self.inner_min_iters = 8      # do at least this many inner iterations before testing tol
    ...
    def regularise_contact_separation(
        ...
        try:
            n_nodes = int(self._regularisation_stats.get("n_nodes", len(vals_reg))) or len(vals_reg)
            ...
            self._regularisation_stats["ever_active"] = self._regularisation_stats.get("ever_active", False) or ever
            self._regularisation_stats["last_count"] = original_count
            self._regularisation_stats["last_fraction"] = float(frac)
            self._regularisation_stats["max_fraction"] = max(self._regularisation_stats.get("max_fraction", 0.0), float(frac))
            self._regularisation_stats["last_saturated_fraction"] = float(saturated_frac)
    ...
    def soderfjall_solve(
        ...
        *,
        ...
        if not full:
            ...
            if not getattr(self, "_beta_freeze_active", False):
                ...
                if (getattr(self, "_relax_k_counter", 0) % k) == 0:
                    ...
                    if hasattr(self, "prev_pressure_residual"):
                        allow_increase = (self.prev_pressure_residual is None) or np.isfinite(self.prev_pressure_residual)

                    # EMA smoothing
                    ema_alpha = 0.2  # smaller = smoother beta
                    proposed = (1.0 - ema_alpha) * self._beta_ema + ema_alpha * target_beta
                    ...
                    # print(f"[BETA] New beta {proposed:.6g}, old {float(self.beta):.6g}")
            ...
            print(f'Current Beta: {float(self.beta):.6g}, beta freeze active: {self._beta_freeze_active}, full {full}')
            self.fenicssolve()
        else:
            ...
            p_sol = self.p.vector()[:]
            beta_frac = 0.01
            self.set_beta(np.max(abs(p_sol)) * beta_frac)
    ...
    def balance_equation(self, eccentricity, HMMState=False, transientState=False):
        ...
        print(f"Minimum film thickness mm: {self.h.vector().min()*self.material_properties.c}")
    ...
    def _smooth_penalty(self, frac: float, *, f0: float = 0.005, width: float = 0.01,
                        gain: float = 5.0, eps: float = 1e-4) -> float:
        """
    ...
    def _apply_regularisation_guard(self, resid_nd: float) -> float:
        ...
        print(f"[REG-GUARD] active at end: pre_end={frac_pre_end*100:.2f}%, "
    ...
    def EHL_HMM_solve(self, eccentricity, HMMState=True, transientState=False):
        ...
        print(f"Max initial pressure: {self.p.vector().max()*self.material_properties.p0/1e6} MPa")
        print(f"Max initial deformation: {self.delta.vector().max()*self.material_properties.c} mm")
        ...
        with self._suspend_balance_cache():
            while (
                ...
                print(f'Max pressure after Newton: {self.p.vector().max()*self.material_properties.p0/1e6} MPa')
                print(f'Min film thickness after Newton: {self.h.vector().min()*self.material_properties.c/1e-6} um')
        ...
        print(f"Maximum pressure MPa: {self.p.vector().max()*self.material_properties.p0/1e6}")
        print(f"Minimum film thickness um: {self.h.vector().min()*self.material_properties.c/1e-6}")
        ...
        try:
            frac_pre_end = 100.0 * float(self._regularisation_stats.get("last_fraction", 0.0))
            frac_sat_end = 100.0 * float(self._regularisation_stats.get("last_saturated_fraction", 0.0))
            print(f"[REG-GUARD] end-state: pre_end={frac_pre_end:.2f}%, "
                  f"sat_end={frac_sat_end:.2f}% → returned ND residual: {guarded_residual_nd:.3e}")
        except Exception:
            pass
        if not self._cache_suppressed:
            self._balance_cache[key] = (_xi if '_xi' in locals() else None, guarded_residual_nd)
    ...
    def EHL_balance_equation(self, eccentricity, HMMState=False, transientState=False):
        ...
        if self._skip_function_space_setup==False:
            ...
            print(f'Min Film thickness mm: {self.h.vector().min()*self.material_properties.c}')
            self.fenicssolve()
        ...
        # if hasattr(self, "_last_outer_resid"):
        #     try:
        #         if np.isfinite(self._last_outer_resid):
        #             loosen = 0.2 * float(self._last_outer_resid)
        #             pressure_relaxation_tol = max(self.inner_tol_p, min(1e-2, loosen))
        #             deformation_relaxation_tol = max(self.inner_tol_d, min(1e-2, loosen))
        #     except Exception:
        #         pass
        print(f"Using inner tols: p={pressure_relaxation_tol:.2e}, δ={deformation_relaxation_tol:.2e} "
        ...
        print(f"Initial residuals: p={pressure_residual:.2e}, δ={deformation_residual:.2e}")
        ...
        reg_frac_tol = float(getattr(self.solver_params, "reg_clear_fraction_tol", 1e-6))
        reg_clear_streak = 0
        ...
        def _regularisation_active() -> bool:
            ...
            return (getattr(self, "_reg_decay_S_current", 0.0) > 0.0) or (frac_end > reg_frac_tol)
        ...
        with self._suspend_balance_cache():
            while (
                ...
                xi, _ = self.soderfjall_solve(
                    ...
                    transientState=transientState,
                    postprocess=False,
                    full=(relaxation_iteration == 0),
                )

                # Freeze beta once we're 'close' to convergence to avoid jitter
                try:
                    _freeze_tol = getattr(self.solver_params, "beta_freeze_p_tol", 5e-4)
                    _freeze_after = getattr(self.solver_params, "beta_freeze_min_iters", 20)
                except AttributeError:
                    _freeze_tol, _freeze_after = 5e-4, 20

                if (not getattr(self, "_beta_freeze_active", False)) and \
                   (relaxation_iteration >= _freeze_after) and \
                   (prev_p_res < _freeze_tol):
                    self._beta_freeze_active = True
                    self._beta_freeze_just_set = True
                    print(f"[BETA] Freezing beta at {float(self.beta):.6g} "
                ...
                try:
                    trigger = float(self.reg_decay_trigger_factor) * float(pressure_relaxation_tol)
                except Exception:
                    trigger = 2.0 * float(pressure_relaxation_tol)
                if (not self._reg_decay_active) and (pressure_residual <= trigger):
                    ...
                    print(f"[REG-S] Starting smoothing decay: S0={self._reg_decay_S0:.3e}, "
                          f"trigger={trigger:.3e}, iter={relaxation_iteration}")
                if self._reg_decay_active:
                    k = max(int(self.reg_decay_iters), 1)
                    prog = max(0.0, min(1.0, (relaxation_iteration - self._reg_decay_start_iter + 1) / k))
                    ...
                    print(f"[REG-S] iter={relaxation_iteration} "
        ...
        try:
            ...
            if not np.isfinite(polish_res) or (polish_res > max(0.7*pressure_relaxation_tol, pressure_residual)):
                ...
                print("[POLISH] Step rejected (no improvement). Kept mixed-solution state.")
            else:
            ...
            print(f"[POLISH] Skipped due to error: {type(_polish_err).__name__}: {_polish_err}")
        finally:
        ...
        difference = load + self.force
        try:
            self._last_outer_resid = abs(difference[2]) / (abs(load[2]) if abs(load[2]) > 0 else 1.0)
        except Exception:
        ...
        print(f"Maximum pressure MPa: {self.p.vector().max()*self.material_properties.p0/1e6}")
        print(f"Minimum film thickness um: {self.h.vector().min()*self.material_properties.c/1e-6}")
        print(f"Maximum deformation um: {self.delta.vector().max()*self.material_properties.c/1e-6}")
        ...
        try:
            ...
            frac_pre_end = 100.0 * float(self._regularisation_stats.get("last_fraction", 0.0))
            frac_sat_end = 100.0 * float(self._regularisation_stats.get("last_saturated_fraction", 0.0))
            msg = (f"[REG-GUARD] end-state: pre_end={frac_pre_end:.2f}%, "
                   f"sat_end={frac_sat_end:.2f}% → returned ND residual: {guarded_residual_nd:.3e}")
    ...
    def solve_loadbalance_secant(self, HMMState=False, transientState=False):
        ...
        if abs(f0) <= tol:
            ...
            adj_abs = max(1e-6 * self.material_properties.length_ratio, 1e-10)  # absolute floor
        ...
        print(f'Change in deformation norm after final calc: {np.linalg.norm(newdeformation - backupdeformation):.3e}')
    ...
    def local_deformation(self):
        ...
        if not hasattr(self, 'dP'):
            return (self.material_properties.p0/self.material_properties.k_spring)*self.p.vector()[:]
        else:
            return (self.material_properties.p0/self.material_properties.k_spring)*(self.p.vector()[:] + self.dP.vector()[:])
    ...
    def snapshot_state(self):
        ...
        ref["h_regularisation_param"] = float(getattr(self, "h_regularisation_param", 0.0))
    ...
    def compare_loaded_state(self, reference):
        ...
        def _maxrel(a, b):
            ...
            return float(_np.max(_np.abs(a - b) / (denom + 1e-16))) if a.size and b.size else 0.0
        ...
        cur_eccz  = float(getattr(self.material_properties, "eccentricity0", [0,0,0])[2])
        ...
        def _print_scalar(name, cur, ref):
            ...
            print(f"{name:25s}: Δabs={dabs:.3e}, Δrel={drel:.3e}  (cur={cur:.6g}, ref={ref:.6g})")
        ...
        _print_scalar("h_regularisation_param", cur_S, reference.get("h_regularisation_param"))
        ...
        print("\n[pressure]     Δabs_max = %.3e, Δrel_max = %.3e" % (_maxabs(p_cur, p_ref), _maxrel(p_cur, p_ref)))
        print("[deformation]  Δabs_max = %.3e, Δrel_max = %.3e" % (_maxabs(d_cur, d_ref), _maxrel(d_cur, d_ref)))
        print("[film_thickn.] Δabs_max = %.3e, Δrel_max = %.3e" % (_maxabs(h_cur, h_ref), _maxrel(h_cur, h_ref)))
        print("[DIAGNOSTIC] ==================================================\n")
